...
toggleBtn.addEventListener('click', () => {
  ...
  toggleBtn.styleBasq.display = 'none';
});
...
document.addEventListener('click', (event) => {
  if (!sidebar.contains(event.target) && !toggleBtn.contains(event.target)) {
    ...
    toggleBtn.styleBasq.display = 'block';
  }
});
...
bolas.forEach(bola => {
  ...
  bola.addEventListener("mousedown", (e) => {
    ...
    bola.styleBasq.cursor = "grabbing";
  });
  ...
  document.addEventListener("mousemove", (e) => {
    ...
    bola.styleBasq.left = `${Math.max(0, Math.min(newX, maxX))}px`;
    bola.styleBasq.top = `${Math.max(0, Math.min(newY, maxY))}px`;
  });

  document.addEventListener("mouseup", () => {
    isDragging = false;
    bola.styleBasq.cursor = "grab";
  });
});
...
btnIncluir.addEventListener('click', () => {
  modal.styleBasq.display = 'block';
});

fecharModal.addEventListener('click', () => {
  modal.styleBasq.display = 'none';
});

window.addEventListener('click', (e) => {
  if (e.target === modal) modal.styleBasq.display = 'none';
});
...
btnVisualizar.addEventListener('click', () => {
  ...
  listaModal.styleBasq.display = 'block';
});

fecharLista.addEventListener('click', () => {
  listaModal.styleBasq.display = 'none';
});
...
btnEditar.addEventListener('click', () => {
  ...
  if (index !== null && jogadores[index]) {
    ...
    modalEditar.styleBasq.display = 'block';
  } else {
...
fecharEditar.addEventListener('click', () => {
  modalEditar.styleBasq.display = 'none';
});
